...
for year in range(start_year, end_year + 1):
	...
	for month in ['08', '09', '10', '11', '12']:
		monthly_dict = {'Nd': [], 'cwp': [], 'iwp': [], 'cth': [], 'cod': [], 'l_re': [], 'i_re': [], 'cf': [], 'modis_aod': [], 
		...
		for n in range(1, monthrange(int(year), int(month))[-1] + 1):
			...
			try:
				...
				print('\n no ECMWF, skipping', day)
				
				monthly_dict['t2'].append(np.full((180, 360), np.nan))
				monthly_dict['CAPE'].append(np.full((180, 360), np.nan))
